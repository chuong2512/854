...
public static class PlayfabManager
{
	...
	{
		float max=PlayfabManager.titleData.adDragonProbWeightAdGold+PlayfabManager.titleData.adDragonProbWeightAdToken+PlayfabManager.titleData.adDragonProbWeightAdCredits+
		          PlayfabManager.titleData.adDragonProbWeightAdScrap+PlayfabManager.titleData.adDragonProbWeightAdMyth+PlayfabManager.titleData.adDragonProbWeightAdAeon;
	...
	{
		float num=PlayfabManager.titleData.adDragonProbWeightDirectToken+PlayfabManager.titleData.adDragonProbWeightDirectCredits+PlayfabManager.titleData.adDragonProbWeightDirectScrap+
		          PlayfabManager.titleData.adDragonProbWeightDirectMyth+PlayfabManager.titleData.adDragonProbWeightDirectAeon;
	...
	private static void LoginCustom(Action callback,bool showPlayfabCustomLoginWarning)
	{
		...
		delegate(LoginResult res)
		{
			...
			request2,
			delegate(GetPlayerProfileResult profile)
			{
	...
	public static void PrepareForIap(Action<bool> callback)
	{
		try
		{
			...
			{
				PlayfabManager.AskPlayerData(delegate(bool isSuccess,SaveData t1,PlayfabManager.RewardData t2) { callback(isSuccess); });
			}
			else
			{
				UnityEngine.Debug.Log("Store manager is Authenticating. Forced authentication = "+flag);
				StoreManager.Authenticate(flag,delegate { PlayfabManager.Login(delegate { callback(PlayfabManager.HaveLoggedIn()); },true); });
			}
		}
		catch (Exception ex)
		{
	...
	public static void AskTitleNews(Action<List<PlayfabManager.NewsData>> callback)
	{
		try
		{
			...
			delegate(PlayFabError error) { UnityEngine.Debug.Log("Error while Asking title news: "+error.ErrorMessage); },
			null,
			null);
		}
		catch (Exception ex)
	...
	private static void AskPlayerDataAfterLogin(Action<bool,SaveData,PlayfabManager.RewardData> callback)
	{
		...
		req,
		delegate(GetUserDataResult res) { PlayfabManager.OnGotPlayerData(res,callback); },
		delegate(PlayFabError err)
		{
			StoreManager.Authenticate(
			false,
			delegate
			{
				...
				delegate
				{
					...
					{
						PlayFabClientAPI.GetUserData(req,delegate(GetUserDataResult res) { PlayfabManager.OnGotPlayerData(res,callback); },delegate(PlayFabError err2) { callback(false,null,null); },null,null);
					}
				},
				true);
			});
	...
	private static void SaveAfterLogin(UpdateUserDataRequest req,Action onSuccess)
	{
		...
		req,
		delegate(UpdateUserDataResult res)
		{
			if(onSuccess!=null)
			{
				onSuccess();
		...
		delegate(PlayFabError err)
		{
			...
			delegate
			{
				...
				delegate
				{
					PlayFabClientAPI.UpdateUserData(
					req,
					delegate(UpdateUserDataResult r)
					{
						if(onSuccess!=null)
						{
							onSuccess();
	...
	private static void AskTitleDataAfterLogin()
	{
		...
		req,
		delegate(GetTitleDataResult res) { PlayfabManager.OnReceivedTitleData(res); },
		delegate(PlayFabError err)
		{
			...
			delegate
			{
				...
				delegate
				{
					PlayFabClientAPI.GetTitleData(
					req,
					delegate(GetTitleDataResult res)
					{
						UnityEngine.Debug.Log("Playfab: Got titleData at try#2");
						PlayfabManager.OnReceivedTitleData(res);
					},
					delegate(PlayFabError e) { UnityEngine.Debug.Log("Playfab: Error while getting titleData at try#2: "+err.ErrorMessage); },
					null,
					null);
				},
				true);
	...
	private static void SendPlayerEventAfterLogin(string eventName,Dictionary<string,object> body,Simulator sim,Action<WriteEventResponse> resultCallback=null,Action<PlayFabError> errorCallback=null,
		bool fillCommonEventData=true)
	{
		...
		request,
		delegate(WriteEventResponse res)
		{
